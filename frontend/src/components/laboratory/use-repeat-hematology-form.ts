...
/**
 * Hook managing Hematology Repeat Test CRUD operations and form state.
 *
 * Encapsulates data fetching, record selection, create/update logic,
 * and medical personnel dialog coordination for the RepeatHematologyDialog.
 *
 * @see RepeatHematologyDialog — the consumer component
 */

import { useState, useCallback, useEffect, useRef } from "react";
import { httpClient } from "@/lib/http-client";
import { toast } from "sonner";
import type { HematologyRepeatTest, RepeatTestSummary } from "./repeat-hematology-types";
import { EMPTY_HEMATOLOGY_REPEAT } from "./repeat-hematology-types";

// ---------------------------------------------------------------------------
// Types
// ---------------------------------------------------------------------------

export interface UseRepeatHematologyFormOptions {
...
export interface UseRepeatHematologyFormResult {
  /** Current form data. */
  data: HematologyRepeatTest;
  ...
  previousExams: RepeatTestSummary[];
  /** Whether the previous exams list is loading. */
  loadingExams: boolean;

  /** Update a single form field. */
  updateField: (field: keyof HematologyRepeatTest, value: string) => void;
 ...
 */
function sanitizeRecord(record: HematologyRepeatTest): HematologyRepeatTest {
  return Object.fromEntries(
    Object.entries(record).map(([k, v]) => [k, v ?? ""])
  ) as HematologyRepeatTest;
}
...
/**
 * Manages Hematology Repeat Test form state with full CRUD behavior.
 *
 * State:
 ...
 */
export function useRepeatHematologyForm({
  open,
  laboratoryReportId,
}: UseRepeatHematologyFormOptions): UseRepeatHematologyFormResult {
  // Form state
  const [data, setData] = useState<HematologyRepeatTest>({ ...EMPTY_HEMATOLOGY_REPEAT });
  const [editing, setEditing] = useState(false);
  const [saving, setSaving] = useState(false);
  const [selectedId, setSelectedId] = useState<string | null>(null);

  // Previous exams list
  const [previousExams, setPreviousExams] = useState<RepeatTestSummary[]>([]);
  ...
  const basePath = `/api/laboratory-reports/${laboratoryReportId}/hematology-repeat-tests`;
  ...
  const loadRepeatTest = useCallback(async (id: string) => {
    try {
      const record = await httpClient.get<HematologyRepeatTest>(`${basePath}/${id}`);
      setData(sanitizeRecord(record));
      setSelectedId(id);
      setEditing(false);
    } catch (error: unknown) {
      const message = error instanceof Error ? error.message : "Unknown error";
      console.error("Failed to load repeat test:", message);
      toast.error("Failed to load repeat test");
    }
  }, [basePath]);

  // Reset form state and fetch previous exams when dialog opens.
  // Using a ref to track the previous `open` state avoids calling setState
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (selectedId) {
        const updated = await httpClient.put<HematologyRepeatTest>(
          `${basePath}/${selectedId}`,
          body
        );
        setData(sanitizeRecord(updated));
        toast.success("Repeat test updated successfully");
      } else {
      ...
      const message = error instanceof Error ? error.message : "Unknown error";
      console.error("Failed to save repeat test:", message);
  ...
  const updateField = (field: keyof HematologyRepeatTest, value: string) => {
